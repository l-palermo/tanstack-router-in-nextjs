...
export const useSharedContext = () => {
  ...
  if (!ctx) throw new Error('useSharedContext must be used within SharedContext Provider');
  return ctx;
};
...
export function FeatureContext({ basePath }: { basePath: string }) {
  return function FeatureContextInner() {
    ...
    const hasSubpath = state.location.pathname.replace(basePath, '').length

    useEffect(() => {
      history.push(hasSubpath ? state.location.pathname : `${basePath}/screen-one`);
    }, []);

    return (
      <SharedContext.Provider value={{ count, setCount, basePath }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', paddingTop: 200, gap: 16, height: '100vh' }} >
